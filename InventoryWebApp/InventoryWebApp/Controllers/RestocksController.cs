...
namespace InventoryWebApp.Controllers
{
    public class RestocksController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "RestockId,QuantityPurchased,PurchasePrice,UserId,Products")] Restock restock)
        {
            if (ModelState.IsValid)
            {
                restock.UserId = User.Identity.GetUserId().ToString();
                restock.ProductId = (int)TempData["ProductId"];
                var product = db.Products.Where(p => p.ProductId == restock.ProductId).FirstOrDefault(); 
                product.Quantity += restock.QuantityPurchased;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "RestockId,QuantityPurchased,PurchasePrice,UserId")] Restock restock)
        {
            if (ModelState.IsValid)
            {
